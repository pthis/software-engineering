...
<tr>
	<td>&nbsp;</td>
	<td width="50%"><font color="#FFFFFF" size="+3">网站用户信息导入&gt;教师信息导入</font></td>
...
END;
			...
			$items[] = Array('tid' => $tid,'password' => $password,'name' => $name,);
		}
	...
	else
	{
		...
		//$query = "INSERT INTO student ( tid, password, name ) VALUES ";
		foreach( $items as $item )
		{
	
			$tid = addslashes( $item['tid'] );
			$password = addslashes( $item['password'] );
			$name = addslashes( $item['name'] );
			//$phoneNO = addslashes( $item['phoneNO'] );
			$value[] = "(NULL,'$tid','$password','$name')";		


		}

		$query = "INSERT INTO teacher (id, tid, password, name ) VALUES ".@implode(',',$value).";";
